...
class Campaigns_Model extends CI_Model
{
	...
	*/
	public function getAll($status = 'all', $limit = null, $offset = 0, $sort_by, $sort_order, $search = 'all')
	{
		...
		$sort_columns = array('id', 'name', 'submit_at', 'created_at');
		$sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'id';

		$result = array();
		$res = FALSE;

		$this->db->select('id, name, status, submit_at, created_at');
		if ($status == 'all') {
...
/* Location: ./application/modules/admin/models/Campaigns_Model.php */ ?>