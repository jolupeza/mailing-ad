...
{
	private $_table = 'mailing_lists';

	/**
	* Método para obtener lista de correo
	*
	* @access public
	...
	*/
	public function getAll($status = 'all', $limit = null, $offset = 0, $sort_by, $sort_order, $search = 'all')
	{
		$sort_order = ($sort_order == 'asc') ? 'asc' : 'desc';
		$sort_columns = array('id', 'name', 'description', 'access_level');
		$sort_by = (in_array($sort_by, $sort_columns)) ? $sort_by : 'id';

		$result = array();
		$res = FALSE;

		$this->db->select('id, name, address, description, access_level, status, created_at');
		if ($status == 'all') {
...
/* Location: ./application/modules/admin/models/Lists_Model.php */ ?>