...
(function($){
  ...
  j('.content').load(_root_ + 'admin/posts/displayPostAjax');
  ...
  j(document).on('click', '#search-btn', function(ev){
    ...
    j('.content').load(href + search + '/' + category);
  });
  ...
  j('.remove-img-featured').on('click', function(ev){
    ...
    j.post(_root_ + 'admin/posts/deleteFeaturedImages', {
      'id' : id
    }, function(data){
      if (data) {
